execute unless score map settings matches 5 run scoreboard players add map settings 1

execute if score map settings matches 0 run tellraw @a[distance=..50] {"text":"No Map selected.", "color":"gold"}
execute if score map settings matches 1 run tellraw @a[distance=..50] {"text":"Map 1 - Four Corners selected. (Extra Large)", "color":"gold"}
...
execute if score map settings matches 5 run tellraw @a[distance=..50] {"text":"Map 5 - Survival Beginnings selected. (Large)", "color":"gold"}
