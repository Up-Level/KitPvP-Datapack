# Class 103 - Infected

execute as @s at @s run particle minecraft:dust 0 0.27058823529 0 1 ~ ~1 ~ 0.25 .35 0.25 0.05 1 force
item replace entity @s armor.chest with minecraft:leather_chestplate{display: {color: 17664}}
item replace entity @s armor.feet with minecraft:leather_boots{display: {color: 17664}, Unbreakable: 1b, Enchantments: [{id: "minecraft:feather_falling", lvl: 2}]}

attribute @s minecraft:generic.movement_speed modifier add 0-0-0-0-0 "SpeedClass" 0 multiply_base
effect give @s minecraft:hunger 1 9 true
effect give @s minecraft:saturation 1 255 true
effect give @s minecraft:strength 1 1 true
